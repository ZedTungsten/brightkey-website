...
function loadLocalDatabaseEnvironment() {
  if (localEnvironmentLoaded || process.env.VERCEL_ENV === 'production') return;
...
export function createAuthenticatedClient(accessToken) {
  ...
  const url = process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_ANON_KEY;
  if (!url || !key || !accessToken) throw new Error('Authenticated database configuration is unavailable.');
  return createClient(url, key, {
...
export async function requireCompanyAccess(req, supabase, companyId, options = {}) {
  ...
  if (isCustomerPortalUser(user)) return { error: 'forbidden' };
  ...
  const ownsTenant = Boolean(
    ownerTenant
    && String(ownerTenant.owner_email || '').trim().toLowerCase() === String(user.email || '').trim().toLowerCase()
  );
  if (memberError || ownerError || (!member && !ownsTenant)) return { error: 'forbidden' };

  const effectiveMember = ownsTenant
    ? { role: 'owner', accessible_modules: [] }
    : member;

  const role = String(effectiveMember.role || '').toLowerCase();
  ...
  const memberModules = (effectiveMember.accessible_modules || []).map(value => String(value).trim().toLowerCase());
  ...
  return { user, company, member: effectiveMember };
}
