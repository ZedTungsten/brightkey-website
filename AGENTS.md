...
> - **Signed-In Chrome Only**: Always use the user's signed-in Chrome browser for authenticated application testing, Supabase migrations, live schema/data verification, and end-to-end workflow validation. Do not use the in-app browser for this project. If signed-in Chrome is unavailable, stop and ask the user to make it available rather than falling back to the in-app browser.
...
> - **Map Physical HTML Files to Clean URLs Deliberately**: With `"cleanUrls": true`, a physical file such as `/dashboard/orders.html` is served publicly as `/dashboard/orders`, and `/dashboard/orders.html` redirects to that extensionless URL. New navigation, canonical links, sidebar aliases, and redirects must use the clean public URL; `.html` belongs only to the repository filename. For a route family with subpages, keep one intentionally named implementation file and rewrite each public subpage to its extensionless clean destination.
> - **Avoid Page-Filename and Subpage-Stem Collisions on New Pages**: When designing a new route family, do not use a physical filename whose clean URL is also the parent stem of rewritten subpages when redirects are involved. For example, pairing `warehouse/inspected.html` with `/warehouse/inspected/in-stock` and a redirect from `/warehouse/inspected` can create ambiguous or circular routing. For a new page, choose a neutral implementation filename such as `warehouse/inspected-page.html`, rewrite subpages to its extensionless clean destination, and keep any legacy parent redirect one-way. Never retrofit this pattern by renaming an existing page without the approval required below.
> - **Support the Actual Local Server**: Identify the process serving localhost before diagnosing route behavior. `vercel dev` applies project routing differently from a plain static server, so do not add physical route shims or restructure pages based on an assumption about the server. If the documented workflow genuinely uses a static server that cannot apply required rewrites, explain the limitation and obtain approval before adding fallback entry files. Verify the exact localhost URL as well as production.
> - **Never Rename or Relocate an Existing Page Without Approval**: A routing or deployment bug does not authorize changing an existing HTML filename, directory, canonical route, or implementation location. First preserve the current page and fix the smallest routing/configuration issue around it. If renaming or moving the page is genuinely required, explain the impact and obtain the user's explicit permission before changing files, links, redirects, rewrites, tests, or bookmarks.
> - **Restart Local Routing Before Restructuring Pages**: `vercel dev` compiles a route manifest when it starts and may not hot-reload `vercel.json` edits, HTML filename changes, or newly added route files even though ordinary CSS/JavaScript changes update normally. When a route works in production but localhost unexpectedly returns 404, identify the process on the local port, restart that same dev server, and retest the exact URL before modifying page structure or adding fallback route files. Do not misclassify localhost as a plain static server until the running process has been verified.
