...
test('Ship Send ignores cancelled SKUs without hiding active lines from the same order', () => {
  assert.match(page, /if \(item\.timestamp_cancelled \|\| String\(item\.status \|\| ''\)\.trim\(\)\.toLowerCase\(\) === 'cancelled' \|\| this\.isServiceSku\(item\.sku\)\) return false/);
...
test('Ship Send removes orders after they are booked for delivery', () => {
  ...
  assert.match(page, /ShipTabCounts\.update/);
});
