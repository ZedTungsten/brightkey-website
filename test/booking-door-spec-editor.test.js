...
test('door type section uses one pencil action and three specification dropdowns', () => {
  ...
  assert.match(details, /data-door-spec="jambMaterial">\$\{window\.bookingDoorSelectOptions\(\['wood', 'PVC', 'metal', 'aluminum', 'glass', 'gate', 'concrete'\]/);
...
test('booking details do not expose the removed dashboard door completion shortcut', () => {
  const combined = `${details}\n${page}\n${assets}\n${styles}`;
  assert.doesNotMatch(combined, /btn-door-done|btn-unallocated-done|completion-proof|CompletionProof|handleUnassignedDoorDoneProof|booking-door-completion-action/);
  assert.match(assets, /window\.triggerFrontageUpload = function/);
});
