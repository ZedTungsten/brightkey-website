...
BEGIN
  SELECT COUNT(DISTINCT tx.reference_id) INTO v_inspect_count
  ...
  SELECT COUNT(DISTINCT tx.reference_id) INTO v_pack_count
  FROM public.inventory_transactions tx
  WHERE tx.company_id = p_company_id
  ...
  SELECT COUNT(DISTINCT tx.reference_id) INTO v_dispatch_count
  FROM public.inventory_transactions tx
  WHERE tx.company_id = p_company_id
  ...
  RETURN QUERY SELECT COALESCE(v_receive_count, 0), COALESCE(v_inspect_count, 0),
    COALESCE(v_pack_count, 0), COALESCE(v_dispatch_count, 0);
END;
$function$;
