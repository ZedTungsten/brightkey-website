(() => {
  ...
  function dragEnd() { state.dragged=null; }
  async function drop(event, scope, targetId) {
    ...
    const [moved]=files.splice(from,1); files.splice(to,0,moved); if(scope==='job')state.byJob[state.selectedJobId]=files; render();
    state.saving=true; const {error}=await persist(); state.saving=false;
    if(error){state.general=previousGeneral;state.byJob=previousByJob;render();console.error('Material order save failed:',error);return showToast('The material order could not be saved. Please try again.',true);}
    showToast('Material order saved.');
  }
  async function saveFile() {
    if(state.saving)return; const source=document.getElementById('material-source').value; const name=document.getElementById('material-name').value.trim(); const url=normalizeUrl(document.getElementById('material-url').value); const type=source==='youtube'?'youtube':document.getElementById('material-file-type').value;
  ...
  window.HRMaterials=Object.freeze({selectJob,openModal,closeModal,changeSource,saveFile,openDelete,closeDelete,deleteFile,openViewer,closeViewer,navigateViewer,dragStart,dragOver,dragEnd,drop});
