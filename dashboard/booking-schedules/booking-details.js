    ...
    async function showBookingDetails(id) {
      ...
      if (rowCount === 0) {
        ...
        for (let i = 0; i < doorsArr.length; i++) {
          ...
          let productCellHtml = `
            <div class="booking-door-title">Door ${i + 1}</div>
          `;
          ...
          const doorTypeHtml = `
            <div class="booking-door-spec-section">
              ...
              <div class="booking-door-spec-editor" id="door-spec-edit-${i}" hidden>
                ...
                <label class="booking-door-spec-field">
                  <span class="booking-door-spec-label">Swing</span>
                  <select class="form-input" data-door-spec="swing">${window.bookingDoorSelectOptions(['Left swing', 'Right swing', 'Sliding', 'Barn Door', 'Double Doors'], door?.swing || 'N/A')}</select>
                </label>
