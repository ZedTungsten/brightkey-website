    ...
    const JournalPartnerReveal = { loadingIds: new Set(), isEnabled(app) { const f = app.getFilters(); return Boolean(f.search || f.selectedAccounts.size); },
      entryNumberHtml(app, row, entryNumber) { if (!this.isEnabled(app) || row._isFilterPartner) return fmtEntry(entryNumber); const loading = this.loadingIds.has(row.id), visible = app.entries.some(item => item._partnerForId === row.id), label = `${visible ? 'Hide' : 'Show'} the paired debit or credit row`; return `<span class="entry-number-wrap"><button type="button" class="entry-pair-btn" onclick="event.stopPropagation();JournalPartnerReveal.reveal(${row.id})" ${loading ? 'disabled' : ''} title="${label}" aria-label="${label}"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><circle cx="9" cy="9" r="5.5"/><circle cx="15" cy="15" r="5.5"/></svg></button><span>${fmtEntry(entryNumber)}</span></span>`; }, async reveal(sourceId) {
        if (!this.isEnabled(JournalApp) || this.loadingIds.has(sourceId)) return; const revealedIndex = JournalApp.entries.findIndex(row => row._partnerForId === sourceId); if (revealedIndex !== -1) { JournalApp.entries.splice(revealedIndex, 1); JournalApp.renderTable(); return; } const source = JournalApp.entries.find(row => row.id === sourceId); if (!source || source._isFilterPartner) return; this.loadingIds.add(sourceId); JournalApp.renderTable();
        try { const params = new URLSearchParams({ select: 'id,entry_number,year,month,date,account_id,account,debit,credit,description_1,description_2,attachments', company_id: `eq.${JournalApp.companyId}`, entry_number: `eq.${source.entry_number}`, id: `neq.${source.id}`, order: 'id.asc', limit: '10' }); const rows = await sbGet(`general_journal?${params}`); const sourceHasDebit = Number(source.debit || 0) !== 0; const partner = rows.find(row => sourceHasDebit ? Number(row.credit || 0) !== 0 : Number(row.debit || 0) !== 0) || rows[0]; if (!partner) { Toast.error('No paired journal row was found for this entry.'); return; } const index = JournalApp.entries.findIndex(row => row.id === sourceId); const insertBefore = JournalApp.sortDir === 'desc' ? partner.id > source.id : partner.id < source.id; if (index !== -1 && !JournalApp.entries.some(row => row.id === partner.id)) JournalApp.entries.splice(index + (insertBefore ? 0 : 1), 0, { ...partner, _isFilterPartner: true, _partnerForId: sourceId }); }
    ...
    const JournalApp = {
      ...
      renderTable() {
        ...
        tbody.innerHTML = this.entries.map((r) => {
          ...
          const rowClasses = r._isFilterPartner ? ['row-filter-partner'] : []; if (this.entries.some(row => row._partnerForId === r.id)) rowClasses.push('row-filter-pair-origin'); if (isDel) rowClasses.push('row-pending-delete'); else if (hasDiff) rowClasses.push('row-pending-edit'); else if (orphan) rowClasses.push('row-orphan'); else if (isCatDeleted) rowClasses.push('row-deleted-cat'); const rowCls = rowClasses.join(' ');
