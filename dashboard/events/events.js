...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!isHolidayPage) EventsApp.init();
});
...
(() => {
  ...
  window.HolidayApp = {
    ...
    bindEvents() {
      ...
      document.getElementById('holidays-table-body')?.addEventListener('click', (event) => {
        const row = event.target.closest('[data-holiday-id]');
        if (row) this.openModal(row.dataset.holidayId);
      });
    ...
    async loadHolidays() {
      ...
      tbody.innerHTML = '<tr class="shimmer-row"><td>Loading...</td><td></td><td></td><td></td></tr>';

      try {
        const { data, error } = await window.BKAuth.sb
          .from('company_holidays')
          .select('id, holiday_name, holiday_type, date_from, date_to, calendar_year, inactive_from_year')
          .eq('company_id', this.companyId)
    ...
    renderTable() {
      ...
      if (!this.holidays.length) {
        tbody.innerHTML = `<tr><td colspan="4" class="holiday-empty-cell">No holidays added for ${this.currentYear}.</td></tr>`;
        return;
      }

      tbody.innerHTML = this.holidays.map((holiday) => `<tr class="holiday-row" data-holiday-id="${esc(holiday.id)}" title="Select to edit">
        <td>${esc(holiday.holiday_name)}</td>
        <td>${esc(HOLIDAY_TYPES[holiday.holiday_type] || holiday.holiday_type)}</td>
        <td>${holiday.calendar_year === this.currentYear ? formatDate(holiday.date_from) : '—'}</td>
        <td>${holiday.calendar_year === this.currentYear ? formatDate(holiday.date_to) : '—'}</td>
      </tr>`).join('');
    },

    renderError(message) {
      const tbody = document.getElementById('holidays-table-body');
      if (tbody) tbody.innerHTML = `<tr><td colspan="4" class="holiday-error-cell">${esc(message)}</td></tr>`;
    },
    ...
    openModal(holidayId = null) {
      ...
      if (holiday) {
        ...
        const belongsToYear = holiday.calendar_year === this.currentYear;
        document.getElementById('holiday-date-from').value = belongsToYear ? (holiday.date_from || '') : '';
        this.toggleDateRange(belongsToYear && Boolean(holiday.date_to));
        document.getElementById('holiday-date-to').value = belongsToYear ? (holiday.date_to || '') : '';
      }
  ...
  document.addEventListener('DOMContentLoaded', () => window.HolidayApp.init());
})();
